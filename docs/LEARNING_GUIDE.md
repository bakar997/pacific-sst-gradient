# Understanding and defending the Pacific SST analysis

For Muhammad Abubakar: use this guide to understand, run, and explain the project before a University of Bergen PhD interview. The project studies an observed sea-surface temperature (SST) contrast. It does not identify causes or run climate-model simulations.
...
## 6. Practise these interview questions

1. **What is your research question?** How did the observed equatorial Pacific west-minus-east SST contrast change, and how sensitive are the estimates to dataset and analysis choices?
...
9. **Have you established a cause?** No. This is an observational description with robustness checks.
10. **What did you verify personally?** Identify the calculations and checks you performed, and explain their outcomes.

## 7. Build your own understanding through execution

Keep a dated log of commands, settings, outputs, expectations, observed changes, and explanations.

- Run the documented setup and full analysis commands. Record software versions and input-file identities.
- Inspect one input: identify temperature units, longitude convention, calendar, missing values, and first and last months.
- Recalculate one monthly box mean from saved weights and compare it with the pipeline output.
- Check a leap-year annual mean manually from its twelve monthly values.
- Change ±3° to ±5°. Predict the effect before comparing results.
- Compare raw and baseline-centred annual series; verify that subtracting a constant preserves the slope.
- Repeat the bootstrap with the same seed, then another seed and block lengths three and seven.
- Explain one figure aloud without notes. Write down uncertainties you cannot yet explain.

## 8. Describe your verification

Keep descriptions of your work tied to the checks you actually performed. Record the commands, calculations, results and methodological choices you can explain.

Use the personal verification log in `docs/ASSISTANCE.md` to document completed checks and questions that remain open.
